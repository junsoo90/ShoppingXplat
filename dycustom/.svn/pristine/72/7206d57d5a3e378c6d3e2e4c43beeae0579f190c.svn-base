...
<script>
	...
	function customDelete(){
		...
		$.ajax({
            ...
            type : 'POST',
            data : data,
            url : "customDelete.ajax",
            
            success : function(data) {
            	alert("회원정보가 삭제 되었습니다");
               	location.href = "${pageContext.request.contextPath}${sessionScope.url }";
            },
...
<script>
	...
	var overlap = false;
  	...
  	function passCheck(url, cId, cInputOldPass){
  		var cChangePass = $("#cChangePass").val(); 
  		if(cInputOldPass == null || cInputOldPass == ""){
  			alert("기존 비밀번호를 입력해주세요");
  			return false;
  		}
  		var bol = true;
