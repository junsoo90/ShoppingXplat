...
<script>
	...
	function deliveryConfirm(purchaseSeq){
		...
		$.ajax({
			type: "POST",
	        url: "deliveryConfirm.ajax",
	        data: {"purchaseSeq":purchaseSeq},
...
<!-- 환불 -->
<script>
	function refundReqeust(purchaseSeq,pOption, index){
		...
		$.ajax({
            ...
            success : function(data) {
            	if(data == 'false' || !data){
	        		...
	        		var cartState = "cartState" + index;
	        		document.getElementById(cartState).innerHTML = "환불요청";
            	}
	...
	function refundCancel(purchaseSeq,pOption,index){
		var data = {
				"purchaseSeq":purchaseSeq,
				"pOption":pOption
		};
		...
		$.ajax({
            ...
            success : function(data) {
            	if(data == 'false' || !data){
	        		...
	        		var cartState = "cartState" + index;
	        		document.getElementById(cartState).innerHTML = "환불요청취소";
            	}
            },
            error:function(request,status,error){
                alert("code:"+request.status+"\n"+"message:"+request.responseText+"\n"+"error:"+error);
	...
	<div class="order-div">
		...
		<div class="order-form-div">
							...
							<tr>
								...
								<td >
									...
									<div class="select-image">
										<img src="/dyadmin/upload/${orderList.savefilename}" width="50" height="50"
								...
								<td>
									<span id="cartState${status.index }" name="cartState${status.index }">
									...
									<div id="deliveryCheck">
										...
										<c:if test ="${orderList.orderState == '환불요청'}">
											<div id="refundCancelBtn${orderList.purchaseSeq}">
												<input type="button" 
													value="환불요청취소" onclick="refundCancel('${orderList.purchaseSeq}','${orderList.pOption}', '${status.index }');">
											</div>
											
											<div id="refundReqestBtn${orderList.purchaseSeq}"  style="display:none;">
												<input type="button" 
													value="환불요청" onclick="refundReqeust('${orderList.purchaseSeq}','${orderList.pOption}','${status.index }');">
											</div>
										</c:if>
			...
			<div class="order-pay-div">
				<table align ="center">
						<tr>
							<td><a href="${pageContext.request.contextPath}/views/orderList.do?keyField=${keyField }&orderKeyWord=${orderKeyWord }&pageNum=1">처음</a></td>
							<td align="center">${pagingHtml}</td>
							<td><a href="${pageContext.request.contextPath}/views/orderList.do?keyField=${keyField }&orderKeyWord=${orderKeyWord }&pageNum=${endPage}" >끝</a></td>
